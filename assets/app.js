...
function init() {
  ...
  dropZone?.addEventListener('drop', event => {
    ...
    handleWorkbookFile(event.dataTransfer.files?.[0]);
  });
...
async function autoLoadDefaultWorkbook() {
  try {
    ...
    setStatus(`Could not auto-load ${DEFAULT_WORKBOOK_PATH}. Add the workbook at that path in the repo, or choose an .xlsx file manually.`, true);
    const diag = document.getElementById('diagText');
...
async function handleWorkbookFile(file) {
  ...
  if (!/\.xlsx$/i.test(file.name)) {
    setStatus('Please choose an .xlsx workbook.', true);
    return;
  }
  ...
  try {
    ...
    await handleWorkbookBuffer(buffer, file.name, 'manual file selection');
  } catch (error) {
    console.error(error);
...
function hydrateMeta() {
  ...
  document.getElementById('selectedCount').textContent = state.selectedRoundIds.length || '—';
      ...
      <section class="card results-card">
        <div class="card-header">
          <div class="class-title">
            ...
            <div class="header-main">Season Points Standings</div>
            <div class="driver-sub">All workbook events are included. Round selector is ignored for this page.</div>
          </div>
    ...
    <section class="card results-card" data-class="${escapeHtml(cls)}">
      <div class="card-header">
        <div class="class-title">
          ...
          <div class="driver-sub">All events · net total after ${points.dropCount} lowest point day${points.dropCount === 1 ? '' : 's'} dropped</div>
        </div>
        <div class="class-count">${rows.length} DRIVER${rows.length === 1 ? '' : 'S'}</div>
      </div>
      <div class="card-body">
      ...
      <section class="card results-card compare-shell insights-shell">
        <div class="card-header">
          <div class="class-title">
            ...
            <div class="driver-sub">Season analysis uses all workbook events. Round selector is ignored for this page.</div>
          </div>
...
function renderDriverInsightContent(data, selectedDriver, selectedRivals = []) {
  ...
  return `
    <div class="insight-section">
      ...
      ${assumptionNote([
        'Uses all workbook events and ignores the round selector.',
...
function renderClosestBattles(data, selectedDriver) {
  ...
  if (!battles.length) {
    return `<div class="compare-empty">No same-group drivers found within 1.000 second across the workbook.</div>`;
  }
      ...
      <section class="card results-card compare-shell">
        ...
        <div class="card-body">
          ...
          ${assumptionNote([
            ...
            'When exactly two drivers are selected, the trend chart uses per-event season points across all workbook events, before drops.'
          ])}
